...
def main_page():
    ui.add_head_html("""<style>
        ...
        .card-s { border-radius: 16px !important; box-shadow: 0 1px 4px rgba(0,0,0,0.04); }
        .card-s:hover { box-shadow: 0 4px 16px rgba(0,0,0,0.07); }
        .hero-bg { background: linear-gradient(135deg, #1e3a5f 0%, #2563eb 100%); border-radius: 20px !important; }
        .st-done { background: #dcfce7; color: #166534; }
        .st-wait { background: #f1f5f9; color: #64748b; }
        .summary-ok { background: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 12px; }
        .summary-err { background: #fef2f2; border: 1px solid #fecaca; border-radius: 12px; }
        .est-chip { background: #f1f5f9; color: #475569; padding: 2px 10px; border-radius: 12px; font-size: 11px; }
        .mode-card { border: 2px solid transparent; cursor: pointer; transition: all 0.15s; }
        ...
        with ui.tab_panel(t0):
            with ui.column().classes("w-full max-w-5xl mx-auto py-6 gap-6"):
                with ui.card().classes("w-full hero-bg text-white p-8"):
                    ui.label("AI Scenario Analysis").classes("text-3xl font-bold mb-1")
                    ui.label(
                        "Generate scenario reports in 4 steps. "
                        "Configure each step, run it, then move to the next."
                    ).classes("text-sm opacity-80 mb-4")
                    ui.button("Get Started  →", icon="play_arrow",
                        on_click=lambda: tabs.set_value("a1")
                    ).props("color=white text-color=blue-8 unelevated")
        ...
        def make_step_tab(panel, code, phases, locked_note, section, actions, extra_content=None):
            ...
            with ui.tab_panel(panel):
                with ui.column().classes("w-full max-w-4xl mx-auto py-6 gap-5"):
                    ...
                    # Header
                    st, detail, mtime = step_status(code)
                    with ui.card().classes(f"w-full card-s bg-{info['color']}-50 border border-{info['color']}-200 p-5"):
                        with ui.row().classes("items-center gap-4"):
                            with ui.element("div").classes(
                                f"w-12 h-12 rounded-xl bg-{info['color']}-500 flex items-center justify-center"
                            ):
                                ui.icon(info["icon"], size="md").classes("text-white")
                            with ui.column().classes("gap-0 flex-grow"):
                                ui.label(f"Step {code} — {info['title']}").classes("text-xl font-bold text-gray-800")
                    ...
                    with ui.card().classes("w-full card-s p-5"):
                        ...
                        with ui.row().classes("gap-3 flex-wrap items-center"):
                            for i, (lbl, icn, key, is_outline, est) in enumerate(actions):
                                ...
                                props = f"color={info['color']} size=md"
                                if is_outline:
                                    props += " outline"
                                if not can:
                                    props += " disable"
